...
class AnimeiatProvider : MainAPI() {
    ...
    // NOTE: reqHeaders is built as a function to avoid the $mainUrl initializer issue
    // (mainUrl is an override var; evaluating it at property-init time may give wrong value)
    ...
    private fun mapSpShow(obj: JSONObject): ShowItem? {
        ...
        if (id <= 0) return null
        // SP API: 'name' is the show title, 'pref' is description/subtitle
    ...
    private suspend fun getMergedShows(): List<ShowItem> = coroutineScope {
        ...
        if (all.isEmpty() && cache != null) return@coroutineScope cache
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse = coroutineScope {
        ...
        section("Anime", merged.filter { it.tags.any { t -> t.contains("أنمي") || t.lowercase().contains("anime") } }.take(40))
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (isSp) {
            ...
            spArr?.let { arr ->
                // SP API: episode title is in 'pref' field; 'name' is always empty
    ...
    override suspend fun loadLinks(
        ...
        Log.d("Cartoony", "loadLinks data in: $data")

        val cleanData = if (data.contains("sp:")) {
            "sp:" + data.substringAfter("sp:")
        } else if (data.contains("leg:")) {
            "leg:" + data.substringAfter("leg:")
        } else {
            data
        }


        val isSp = cleanData.startsWith("sp:")
        val raw = cleanData.substringAfter(":")

        // تحليل المتغيرات: v=<vid>&e=<epId>&s=<showId>
        ...
        if (isSp) {
            if (episodeId != null) {
                val body = mutableMapOf("episodeId" to episodeId.toString())
                if (showId != null) body["showId"] = showId.toString()

                try {
                    val res = app.post("$mainUrl/api/sp/episode/link", headers = buildHeaders(), data = body).text
                    val decryptedTxt = decryptEnvelope(res)

                    if (decryptedTxt != null) {
                        val obj = JSONObject(decryptedTxt)
                        val direct = obj.optString("link", "").trim().takeIf { it.startsWith("http") }
                        val cdn = obj.optString("cdn_stream_private_id", "").trim().takeIf { it.isNotBlank() && it.lowercase() != "null" }

                        if (direct != null) {
                            callback(ExtractorLink(name, "$name Direct", direct, "", Qualities.Unknown.value, direct.contains(".m3u8")))
                            linkFound = true
                        }
                        if (cdn != null) {
            ...
            if (!linkFound && videoId != null && videoId.length > 10) {
                val pegUrl = "https://pegasus.5387692.xyz/api/sp/hls/$videoId/playlist.m3u8"
                callback(ExtractorLink(name, "$name Pegasus", pegUrl, "", Qualities.Unknown.value, true))
                linkFound = true
            }
        } else {
            ...
            if (!linkFound && episodeId != null && showId != null) {
                try {
                    val legTxt = legacyGet("episode?episodeId=$episodeId&showId=$showId")
                        ?: legacyGet("episode?episodeId=$episodeId")

                    if (legTxt != null) {
                        val obj = JSONObject(legTxt)
